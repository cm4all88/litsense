...
function Overview({ onNav }) {
  ...
  useEffect(() => {
    ...
    // Load live Stripe revenue stats for the overview
    stripeAdmin("stats").then(d => setRevenue(d)).catch(() => {});
  }, []);
  ...
  const revenueCards = revenue ? [
    { label:"MRR",      value:fmtMoney(revenue.mrr),   sub:"monthly recurring",  color:T.green  },
    { label:"ARR",      value:fmtMoney(revenue.arr),   sub:"annual run rate",    color:T.blue   },
    { label:"Active",   value:revenue.active,          sub:"paying subscribers", color:T.amber  },
    { label:"Trialing", value:revenue.trialing,        sub:"in free trial",      color:T.purple },
  ] : null;
  ...
  return (
    <div>
      ...
      {revenueCards && (
        <>
          <h3 style={{ margin:"0 0 10px", fontSize:11, fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase", color:T.low }}>Live Revenue</h3>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:10, marginBottom:24 }}>
            {revenueCards.map(c => (
              <div key={c.label} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:10, padding:"14px 16px" }}>
                <div style={{ fontSize:10, fontWeight:600, letterSpacing:"0.07em", textTransform:"uppercase", color:T.mid, marginBottom:8 }}>{c.label}</div>
                <div style={{ fontSize:22, fontWeight:800, color:c.color, fontFamily:"'Fraunces',Georgia,serif", lineHeight:1 }}>{c.value}</div>
      ...
      {/* App stats */}
      <h3 style={{ margin:"0 0 10px", fontSize:11, fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase", color:T.low }}>App Status</h3>
